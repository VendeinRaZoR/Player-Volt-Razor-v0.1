...
#define SPI_PORTX   PORTC
#define SPI_PORTXX   PORTB
#define SPI_DDRX    DDRC
#define SPI_DDRXX    DDRB
...
#define SPI_DisableSS_m(ss)  SPI_PORTX |= (1<<(ss)); 

/*разрешить SPI устройство*/
#define SPI_EnableSS_m(ss)   SPI_PORTX &= ~(1<<(ss)); 

/*статус SPI устройства*/
#define SPI_StatSS_m(ss)    (!(SPI_PORTX & (1<<(ss))))

/*передать байт данных по SPI*/
void SPI_WriteByte_m(unsigned char data) 
{
  SPDR = data;
   while(!(SPSR & (1<<SPIF)));    
}
