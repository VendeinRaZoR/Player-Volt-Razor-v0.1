...
#define lcd_gotoxy_parallel(x, y) parallel_data(0x80|((x)+((y)*0x40)), 0)

void parallel_data(char data, unsigned char rs)
{
if(rs == 1)PORTB.4 = 1;
else PORTB.4 = 0;
PORTB.5  = 1;
if(data & 0x80) PORTB.0  = 1;
else PORTB.0  = 0;
if(data & 0x40) PORTB.1  = 1;
else PORTB.1  = 0;
if(data & 0x20) PORTB.2  = 1;
else PORTB.2  = 0;
if(data & 0x10) PORTB.3  = 1;
else PORTB.3  = 0;
PORTB.5  = 0;
delay_us(100);
...
PORTB.5  = 1;
if(data & 0x08) PORTB.0  = 1;
else PORTB.0  = 0;                
if(data & 0x04) PORTB.1  = 1;
else PORTB.1  = 0;
if(data & 0x02) PORTB.2  = 1;
else PORTB.2  = 0;
if(data & 0x01) PORTB.3  = 1;
else PORTB.3  = 0;
PORTB.5  = 0;
delay_us(100);
PORTB &= ~(PORTB);
...
delay_us(1500);// опциональная задержка
}
...
void lcd_puts_parallel(char *str)
{
unsigned char i = 0;
 
while(str[i])
parallel_data(str[i++], 1);
}
